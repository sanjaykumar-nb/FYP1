...
projects), the untuned, zero-parameter graph rule reaches F1 = 0.712 (95\% bootstrap
CI [0.610, 0.814]), exceeding a logistic-regression model tuned on the training
split (F1 = 0.593) -- a result that holds even at the pessimistic end of the
confidence interval. Three independent attempts to improve on this simple rule with
more sophisticated methods all failed or backfired on held-out data, a set of
negative results we report in full because they are what make the headline claim
credible. A controlled ablation further shows the token-efficient graph architecture
...
results, showing that a zero-parameter deterministic rule matches or exceeds a tuned
learned model on unseen data -- and a controlled ablation showing this efficiency does
not cost detection accuracy.
...
On the 270-sprint, 22-project held-out set -- the headline result -- the untuned,
zero-parameter graph rule achieves precision 0.552, recall 1.000, F1 0.712, against a
composite logistic-regression model (tuned via three-fold group cross-validation on
the training split) achieving precision 0.620, recall 0.568, F1 0.593, AUC 0.658
(Fig.~\ref{fig:holdout}). The system recalls every delayed sprint at the cost of
over-flagging roughly half the on-time ones: a high-sensitivity early-warning
operating point, appropriate when a missed risk is costlier than a dismissible false
alarm.
...
Notably, even the pessimistic end of the F1 interval (0.610) exceeds the tuned
composite model's point estimate (0.593), so the headline claim holds under
resampling uncertainty, not only at the original point estimate.
...
failures; among calls that reached the model, the observed schema-validity rate was
approximately 92\%.

\section{Discussion}

The results support a precise claim, and we are careful not to extend it further: a
simple, explainable, zero-parameter graph rule matches or exceeds a tuned learned
model on unseen projects, at a fraction of the token cost of naive LLM prompting, and
without loss of detection accuracy in a controlled ablation. This is a narrower claim
than ``deterministic graph rules are always superior to learned models'' -- the
result reflects this specific problem's structure (a low-parameter, sprint-level
delay signal, evaluated with strict project-level held-out discipline), not a general
argument against learning.
...
We list ten threats explicitly rather than allowing them to remain implicit.
...
substitute for evaluation on an independent dataset. (8)~Fine-grained
role-based access control is designed but not enforced; the present guarantee is
authentication plus organization-scoping only. (9)~Two designed agents (Meeting
...
(10)~We have not conducted a longitudinal study showing that a flagged risk predicts
...
zero-parameter graph rule matches or exceeds a tuned learned model on unseen
projects, a claim that holds under bootstrap resampling and that a controlled
ablation shows is not an artifact of an easy comparison. We reported three negative
