...
from ai_service.app.prompts.review_trio import FRONTEND_REVIEW_PROMPT


class FrontendAgent(BaseAgent[dict, AgentOutput]):
    def __init__(self, llm_client: GroqClient, fallback: RuleBasedFallback):
        super().__init__("frontend_review")
        self.llm = llm_client
        self.fallback = fallback
    
    def get_system_prompt(self) -> str:
        return FRONTEND_REVIEW_PROMPT
    ...
    async def run(self, input_data: dict) -> AgentOutput:
        ...
        try:
            response = await self.llm.generate_structured(
                ...
                user_prompt=f"Review this proposal from a frontend perspective: {context}",
                output_schema=AgentOutput.model_json_schema(),
            )
            return AgentOutput(**response)
        except Exception as e:
            return self.fallback.frontend_review(input_data)